...
t_test_combo.lm <- function(object, hypotheses, alternatives, alpha = 0.05)
{
  ...
  for(i in 1:H_count)
  {
    ...
    if (Hs_1[i] == "not.equal")
    {
      T_interval_i = c(-Inf, -round(Qs_t_2[1],4), round(Qs_t_2[1],4), Inf, signif(Qs_t_2[2],2))
    ...
    {
      T_interval_i = c(NA, NA, round(Qs_t_1[1],4), Inf, signif(Qs_t_1[2],2))
      T_interval = rbind(T_interval, T_interval_i)
      T_stats_pval = append(T_stats_pval, round(T_i,4))
    } else if(Hs_1[i] == "less")
    {
      T_interval_i = c(-Inf, -round(Qs_t_1[1],4), NA, NA, signif(Qs_t_1[2],2))
      T_interval = rbind(T_interval, T_interval_i)
      T_stats_pval = append(T_stats_pval, -round(T_i,4))
    }
  }

  P_value = signif(2^Tsides_alt*mpt(T_stats_pval, df),4)
  names(P_value) = c("estimate", "error")

  dimnames(T_interval)[[1]] = unname(Hs_0[1,])
  dimnames(T_interval)[[2]] = c("n.bound.l","n.bound.u","p.bound.l","p.bound.u", "error")
